...
FIRST_APP_ENDPOINT = "http://localhost:5000/data-dict"
FIRST_APP_ENDPOINT1 = "http://localhost:5000/latest-data"
...
def analyze_crypto(series1_close, series2_close):
        ...
        spread_zscore_scaled = (spread_scaled - spread_scaled.mean()) / spread_scaled.std()
...
def conditional_negative(z_scores, series1, series2, stoploss, takeprofit, threshold): #threshold has to be a negative number
        profit = 0
        in_position = False
        trades = 0
        wins = 0
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            for combo in combinations(data.keys(), 2):
                ...
                p_value, latest_spread, latest_scaled, crossings, latest_crossings, beta, mean, std, high ,low, same_sign, spread_zscore = analyze_crypto(series1_close, series2_close)
                ...
                if same_sign == 1 and p_value<0.05 and crossings>crossing_opt and latest_crossings>1 and adf < 0.05:
                    ...
                    zero_crossings_len = find_latest_zerocrossings_index(spread_zscore)
                    ...
                    opt_p_z_2nd = 0
                    while k < high:
                        winrate_p, trades_p, profit_p = conditional_positive(spread_zscore, series1_close, series2_close, k+1, 0, k)
                        ...
                        k += 0.1
                    print(f"Best threshold: {opt_p_z}, w/r: {winrate}, t/o: {trades}")
                    print(f"Second best threshold: {opt_p_z_2nd}, w/r: {winrate_2nd}, t/o: {trades_2nd}")
                    ...
                    opt_n_z_2nd = 0
                    while l > low:
                        winrate_n, trades_n, profit_n = conditional_negative(spread_zscore, series1_close, series2_close, l-1, 0, l)
                        ...
                        l -= 0.1
                    print(f"Best threshold: {opt_n_z}, w/r: {winrate_j}, t/o: {trades_j}")
                    print(f"Second best threshold: {opt_n_z_2nd}, w/r: {winrate_j_2nd}, t/o: {trades_j_2nd}")


                    if (opt_p_z != 0 or opt_p_z_2nd != 0 or opt_n_z != 0 or opt_n_z_2nd != 0):
                        global market_candidates
                        with data_lock:
                            exists = any(candidate['series1'] == combo[0] and candidate['series2'] == combo[1] for candidate in market_candidates["candidates"])
                            ...
                            if not exists:
                                market_candidates["candidates"].append({
                                    ...
                                    "half_life": half_life,
                                    "opt_p_z": opt_p_z,
                                    "opt_p_z_2nd": opt_p_z_2nd,
                                    "opt_n_z": opt_n_z,
                                    "opt_n_z_2nd": opt_n_z_2nd,
                                    "beta": beta,
                                    "mean": mean,
                                    "std": std,
            ...
            with data_lock:
                ...
                for candidate in market_candidates["candidates"]:
                    ...
                    if latest_data[series1][0] != series1_close_time[-1][0]:
                        candidate["series1_close_time"].append(latest_data[series1])
                    else:
                        candidate["series1_close_time"][-1] = latest_data[series1]
                    

                    if latest_data[series2][0] != series2_close_time[-1][0]:
                        candidate["series2_close_time"].append(latest_data[series2])
                    else:
                    ...
                    series1_close = [float(item[1]) for item in candidate["series1_close_time"]]
                    series2_close = [float(item[1]) for item in candidate["series2_close_time"]]
                    ...
                    print(f"updating z_score: {candidate['current_z']} to {current_z}")
                    ...
                    zero_crossings_time = len_close_time - candidate["zero_crossings_len"]
        ...
        if response.status_code == 200:
            ...
            with data_lock:
                ...
                for position in market_positions["positions"]:
                    ...
                    if latest_data[series1][0] != series1_close_time[-1][0]:
                        position["series1_close_time"].append(latest_data[series1])
                    else:
                        position["series1_close_time"][-1] = latest_data[series1]
                    

                    if latest_data[series2][0] != series2_close_time[-1][0]:
                        position["series2_close_time"].append(latest_data[series2])
                    else:
                    ...
                    series1_close = [float(item[1]) for item in position["series1_close_time"]]
                    series2_close = [float(item[1]) for item in position["series2_close_time"]]
